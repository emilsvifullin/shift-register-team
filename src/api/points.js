...
export async function saveAdminPoint({
  ...
  const result=await supabaseClient.rpc(
    "admin_save_point",
    {
      p_point_id:id,
      p_name:name,
      p_sort_order:Number(sortOrder),
      p_active:active,
      p_advance_enabled:advanceEnabled,
      p_pricing_type:pricingType,
      p_fixed_rate:fixedRate===null || fixedRate===""
        ? null
        : Number(fixedRate),
  ...
  return resultData(result,"Не удалось сохранить ПВЗ");
}
...
export async function addAdminTariff({
  ...
  return resultData(result,"Не удалось добавить тариф");
}
...
export async function updateAdminTariff({
  ...
  return resultData(result,"Не удалось изменить тариф");
}
...
export async function deleteAdminTariff(id){
  ...
  return resultData(result,"Не удалось удалить тариф");
}
