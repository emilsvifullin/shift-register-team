...
/*
  Подсказка отвечает на единственный вопрос, который по кнопке не виден:
  что станет с историей. Что именно сейчас правится, человек только что
  выбрал сам, и повторять это незачем.
*/
export function tariffIntentHelp(intent){
  if(
    ...
    return "Новая запись в истории не создаётся.";
  }

  return "Прежний тариф останется в истории.";
}
